#!/usr/bin/env python3
"""Operational Hatun organ bus. Second Brain handles only. Anatomy observe only.
Winner stays null. Ready stays false. No Hub PUT.
"""
...
DIM = 12
def tokenize(query):
    return [t for t in re.split(r"[^a-z0-9._-]+", query.lower()) if len(t) > 1]
def fnv(token):
    h = 2166136261
    for ch in token:
...
def selftest():
    ...
    ok = (hit["brain"]["action"] == "NAVIGATE" and hit["brain"]["handles"] == ["khipu.handle.alpha"] and hit["brain"]["hydrated"] is False and hit["draft"] == "DRAFT" and hit["admitted"] is False and miss["brain"]["action"] == "ABSTAIN" and bad["brain"]["action"] == "REFUSE" and hit["anatomy"]["can_modify_decision"] is False and hit["winner"] is None)
    return {"selftest": ok, "hit": hit}
def main():
    p = argparse.ArgumentParser(); p.add_argument("--query", default=""); a = p.parse_args()
    if a.query:
        print(json.dumps(run(a.query), indent=2)); return 0
    r = selftest(); print(json.dumps(r, indent=2)); return 0 if r["selftest"] else 2
